...
export function mapPostToCard(post: Post): CardPostData {
  const validCategories = Array.isArray(post.categories)
    ? post.categories.filter((cat): cat is number | Category =>
  ...
  const image = post.heroImage && typeof post.heroImage === 'object' && post.heroImage !== null
    ? { ...post.heroImage, url: post.heroImage.url ?? '', alt: post.heroImage.alt ?? post.title ?? '' }
    : null;
  return {
    id: typeof post.id === 'string' ? post.id : String(post.id ?? ''),
    title: post.title ?? '',
    slug: post.slug ?? '', // Ensure slug is always provided, even if optional in Post
